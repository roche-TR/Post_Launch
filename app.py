...
def save_config(df):
    ...
    for _, row in df.iterrows():
        ...
        if cell:
            ws.update_cell(cell.row, df.columns.get_loc('weight') + 1, row['weight'])
            for m in months:
                col_name = f"target_{m.lower()}"
                col_idx = df.columns.get_loc(col_name) + 1
                ws.update_cell(cell.row, col_idx, row[col_name])
...
if st.session_state.role == "USER":
    with tab_actual:
        ...
        # Sadece weight > 0 olan metrikler
        ...
        if not df_actuals.empty:
            df_display = pd.merge(
                df_map[['category', 'metric']],
                df_actuals, on='metric'
            )
            # Weight 0 olanları filtrele
